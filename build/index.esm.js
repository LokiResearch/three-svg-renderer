import { Vector3, Float32BufferAttribute, Matrix4, Vector2, Color, Raycaster, DoubleSide, Line3, PerspectiveCamera, Mesh } from 'three';
...
import { extend, Shape, Path, Svg, Image, Text, PathArray, Circle, Number as Number$1, G, Color as Color$1, ClipPath, SVG } from '@svgdotjs/svg.js';
...
})(ViewVertexSingularity || (ViewVertexSingularity = {}));
class ViewVertex {
    hash3d = "";
    hash2d = "";
...
class Chain {
    ...
    edges = new Array();
    vertices = new Array();
    visibility = ChainVisibility.Unknown;
...
function singularityForPoint(viewVertex, camera) {
    ...
    if (natures.size === 1) {
        if (viewVertex.viewEdges.length > 2 && (natures.has(ViewEdgeNature.Silhouette) || natures.has(ViewEdgeNature.Boundary))) {
...
function drawPolygon(parent, polygon, options) {
    ...
    if (options.useRandomColors) {
        style.color = Color$1.random().toString();
    }
...
async function getSVGTexture(mesh) {
    return new Promise((resolve, reject) => {
        svgContentFromDataURL(mesh.texture.url)
            ...
            const group = new G({ id: "svg-interface-" + mesh.name });
...
function transformSVGPath(path, matrix) {
    ...
    path.plot(new PathArray(newCmds));
}
/**
...
class HiddenChainPass extends ChainPass {
    ...
    async draw(svg, viewmap) {
        const chains = viewmap.chains.filter(c => c.visibility === ChainVisibility.Hidden);
...
function getLegend(options) {
    const legend = new G({ id: "edges-nature-legend" });
...
export { Chain, ChainPass, ChainVisibility, DrawPass, FillPass, HiddenChainPass, Polygon, SVGDrawHandler, SVGDrawInfo, SVGMesh, SVGRenderInfo, SVGRenderer, SingularityPointPass, TexturePass, ViewEdge, ViewEdgeNature, ViewVertex, ViewVertexSingularity, Viewmap, ViewmapBuildInfo, VisibleChainPass };
